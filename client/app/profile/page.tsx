...
const encryptAndUploadPhotos = async (
  ...
  for (let i = 0; i < photos.length; i++) {
    ...
    const baseIdBytes = new TextEncoder().encode(`photo-${i}-${Date.now()}`);
    const id = toHex(new Uint8Array([...baseIdBytes, ...nonce]));
...
const handleContinue = async () => {
  ...
  try {
    ...
    const { quiltId, patchIds } = await encryptAndUploadPhotos(formData.photos, packageId, sealClient);
    ...
    try {
      const tx = buildProfileMintTx({ ...mintParams, gasBudget: 100000000 });
